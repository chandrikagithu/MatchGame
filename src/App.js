...
class App extends Component {
  ...
  onClickPlayAgain = () => {
    this.setState({score: 0, seconds: 60, GameOver: false})
  }

  changeImage = clickedImage => {
    const {activeImage} = this.state
    const RandomNumber = Math.floor(Math.random() * imagesList.length)
    const filterImage = imagesList[RandomNumber].imageUrl

    this.setState({activeImage: filterImage})
  ...
  render() {
    const {score, seconds, activeTabId, activeImage, GameOver} = this.state
        ...
        {seconds === 0 || GameOver === true ? (
          ...
          <div className="game-container">
            <div className="filter-image-container">
              <img src={activeImage} alt="match" className="filter-image" />
            </div>
